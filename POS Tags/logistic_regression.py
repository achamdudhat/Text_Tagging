...
classification_lr = LogisticRegression(solver='liblinear', max_iter=5000, random_state=42) #logistic regression model
classification_lr.fit(x_train, y_train)

predictions = classification_lr.predict(x_test)

accuracy = accuracy_score(y_test, predictions)
print(f'Cross-validation Accuracy: {accuracy * 100:.2f}%')  # 95.97%
...
predictions_for_testing = classification_lr.predict(vectors)
...
with (open('logistic_output.txt', 'w') as output_for_lr):
    for word, tag in zip(testing_processed, tag_result):
        output_for_lr.write(f'{word} {tag}\n')  # write word tag lines
