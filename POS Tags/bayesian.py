...
classification_bayesian = BernoulliNB()
classification_bayesian.fit(x_train, y_train)

predictions = classification_bayesian.predict(x_test)

accuracy = accuracy_score(y_test, predictions)
print(f'Cross-validation Accuracy: {accuracy * 100:.2f}%') #83.72%

print(classification_report(y_test, predictions))

filename = 'finalized_model.sav'
pickle.dump(classification_bayesian, open(filename, 'wb'))

#For testing bayesian classifier on unlabeled Data:
testing_processed = []
...
predictions_for_testing = classification_bayesian.predict(vectors)
...
with (open('bayesian_output.txt', 'w') as output_for_bayes):
    for word, tag in zip(testing_processed, tag_result):
        output_for_bayes.write(f'{word} {tag}\n')  # write word tag lines
